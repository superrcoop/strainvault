...
function windowLoadInit() {
	...
	if (jQuery().tweet) {
		jQuery('.twitter').tweet({
			...
			template: "<span class=\"tweet_text\">{tweet_text}</span><span class=\"darklinks\">{time}</span>"
		});
	}
	...
	if ($header.length) {
		...
		jQuery($header).on('affixed-top.bs.affix', function () {
			setTimeout(function(){
				...
				// if ($headerWrapper.height() < newHeaderHeight) {
					$headerWrapper.css({height: $header.outerHeight()});
				// }
			}, 510);
		});
...
function windowReadyInit() {
	...
	//add class for section before ragged footer
	jQuery(".footer_variant1").first().prev().addClass('before_ragged_footer');
}
